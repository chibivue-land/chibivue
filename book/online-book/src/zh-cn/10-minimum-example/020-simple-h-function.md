...
<KawaikoNote variant="funny" title="虚拟 DOM 的本质">

“虚拟 DOM”听起来可能很复杂，但它其实只是一个 JavaScript 对象！它用简单的 `{ type, props, children }` 结构来表示 DOM。
...
<KawaikoNote variant="surprise" title="h 函数完成！">

现在你已经可以用 JavaScript 表示 HTML 了！通过嵌套结构，还能创建任意复杂的 UI。
