...
<KawaikoNote variant="question" title="什么是编译器？">

“编译器”就是翻译器。它把便于人类编写的格式（模板）转换为便于机器处理的格式（h 函数）！
...
<KawaikoNote variant="funny" title="模板的本质">

模板最终会转换为 h 函数调用。也就是说，我们之前实现的 h 函数也会在这里派上用场！
