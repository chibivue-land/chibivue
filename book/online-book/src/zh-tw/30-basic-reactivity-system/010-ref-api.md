...
::: tip 前置知識
在閱讀本章之前，我們建議閱讀[響應式最佳化](/zh-tw/30-basic-reactivity-system/005-reactivity-optimization)以了解基於 alien-signals 的最佳化響應式系統的概念。
:::

## ref api 的回顧（和實作）

Vue.js 有各種與響應式相關的 API，其中 ref 特別著名。
即使在官方檔案中，它也作為"響應式核心"名稱下的第一個主題被介紹。
...
- 對 value 屬性的取得/設定操作會觸發 track/trigger。
...
ref 不會產生像 `{ value: x }` 這樣的響應式物件。對 value 的取得/設定操作的 track/trigger 是由 ref 的實作執行的，如果對應於 x 的部分是物件，它就會變成響應式物件。
...
任務非常簡單。我們可以按原樣使用 RefImpl 的實作，並跳過 `toReactive` 部分。
讓我們在閱讀原始碼的同時實作它！
  ...
  setup() {
    ...
    return () =>
      h('div', {}, [
        ...
        h(
          'button', // 渲染更新為 state.value.count 目前持有的值
          { onClick: forceUpdate },
          ['force update !'],
        ),
      ])
...
toRef 是一個為響應式物件的屬性產生 ref 的 API。
...
※ 從 v3.3 開始，toRef 加入了正規化功能。chibivue 不實作此功能。
請查看官方檔案中的簽名以取得更多詳細資訊！（https://vuejs.org/api/reactivity-utilities.html#toref）
