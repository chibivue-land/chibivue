...
在上一章中，我們實作了與 ref 相關的 API。接下來，讓我們談談 computed。
...
官方實作有點複雜，但讓我們從一個簡單的結構開始。

實作它的最簡單方法是每次檢索值時觸發回呼。
...
實際上，我們希望追蹤相依項並在值更改時重新計算。

為了實作這一點，我們使用一種機制，將 `_dirty` 標誌作為排程器作業更新。
`_dirty` 標誌是一個表示值是否需要重新計算的標誌。它在被相依項觸發時更新。
...
我們將此標誌更新為 true，函式被多個相依項觸發，所以我們將其註冊為 ReactiveEffect 的排程器。

這是基本流程。在實作時，有幾個要注意的點，讓我們在下面總結它們。

- 當將 `_dirty` 標誌更新為 true 時，觸發它擁有的相依項。
  ```ts
  if (!this._dirty) {
    this._dirty = true
...
- 如果你想實作 setter，最後實作它。首先，目標是使其可計算。

現在我們準備好了，讓我們實作它！如果下面的程式碼按預期工作，就可以了！（請確保只觸發 computed 相依項！）
...
watch API 有各種形式。讓我們從實作最簡單的形式開始，即使用 getter 函式進行監視。
首先，讓我們目標使下面的程式碼工作。
...
watch 的實作不在 reactivity 中，而在 runtime-core（apiWatch.ts）中。

它可能看起來有點複雜，因為有各種 API 混合在一起，但如果你縮小範圍，實際上非常簡單。
我已經在下面實作了目標 API（watch 函式）的簽名，所以請嘗試實作它。我相信如果你到目前為止已經掌握了響應式的知識，你可以做到！
...
使用 watch 實作來實作 watchEffect 很容易。
