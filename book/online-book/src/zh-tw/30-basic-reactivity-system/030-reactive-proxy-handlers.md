...
::: warning
這裡解釋的實作基於目前草擬的[響應式最佳化](/zh-tw/30-basic-reactivity-system/005-reactivity-optimization)之前的版本。
一旦[響應式最佳化](/zh-tw/30-basic-reactivity-system/005-reactivity-optimization)完成，本章的內容將更新以與其保持一致。
:::

## 不應該是響應式的物件

現在，讓我們解決目前響應式系統的一個問題。
首先，嘗試執行以下程式碼。
...
原因是 `document.getElementById` 獲得的元素被用來產生 Proxy 本身。

當產生 Proxy 時，值變成 Proxy 而不是原始物件，導致 HTML 元素功能的丟失。
...
這允許我們確定物件的型別。雖然有些硬編碼，但讓我們概括這個確定函式。
...
使用 `slice` 的原因是取得 `[Object hoge]` 中對應於 `hoge` 的字串。

然後，讓我們透過使用 `reactive toRawType` 確定物件的型別並進行分支。
跳過為 HTMLInput 產生 Proxy。

在 reactive.ts 中，取得 rawType 並確定將成為 reactive 目標的物件型別。
...
現在我們可以將 HTML 元素放入 Ref 中，讓我們實作 TemplateRef。
...
如果你已經走到這一步，你可能已經看到如何實作它。
是的，只需將 ref 加入到 VNode 並在渲染期間注入值。
...
在原始實作中，它被稱為 `setRef`。找到它，閱讀它，並實作它！
在原始實作中，它更複雜，ref 是一個陣列並且可以透過 `$ref` 存取，但現在，讓我們目標使上面的程式碼工作。

順便說一下，如果它是一個元件，將元件的 `setupContext` 分配給 ref。
（注意：實際上，你應該傳遞元件的代理，但它還沒有實作，所以我們現在使用 `setupContext`。）
...
陣列本質上是物件，所以當加入新元素時，其索引作為鍵傳遞給 Proxy 的 `set` 處理器。
...
換句話說，`length` 已經註冊了一個 effect。所以，我們需要做的就是提取這個 effect 並在設定索引時觸發它。

如果鍵被確定為索引，我們觸發 `length` 的 effect。
當然，可能還有其他相依項，所以我們將它們提取到一個名為 `deps` 的陣列中並一起觸發 effect。
...
我們可以實作它，就好像有一個註冊了 effect 的 `ITERATE_KEY`。
...
目前，在查看 reactive.ts 的實作時，它只針對 Object 和 Array。
...
這些物件被實作為單獨的 Proxy 處理器。它被稱為 `collectionHandlers`。

在這裡，我們將實作這個 `collectionHandlers` 並目標使以下程式碼工作。
...
在 `collectionHandlers` 中，我們為 add，set 和 delete 等方法實作處理器。
這些的實作可以在 `collectionHandlers.ts` 中找到。
...
透過確定 `TargetType`，如果它是集合型別，我們基於這個處理器為 `h` 產生 Proxy。
讓我們實際實作它！

需要注意的一點是，當將目標本身傳遞給 Reflect 的接收器時，如果目標本身設定了 Proxy，可能會導致無限循環。
為了避免這種情況，我們改變結構，將原始資料附加到目標，當實作 Proxy 處理器時，我們修改它以在這個原始資料上操作。
...
嚴格來說，這個實作也應該為正常的響應式處理器完成，但為了最小化不必要的解釋並且因為到目前為止沒有問題，所以省略了。
讓我們嘗試實作它，如果進入 getter 的鍵是 `ReactiveFlags.RAW`，它回傳原始資料而不是 Proxy。

與此同時，我們還實作了一個名為 `toRaw` 的函式，它遞迴地從目標檢索原始資料並最終獲得處於原始狀態的資料。
