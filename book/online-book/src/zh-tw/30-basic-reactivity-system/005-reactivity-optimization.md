...
::: info 關於本章
本章介紹 Vue 3.6 中將引入的基於 [alien-signals](https://github.com/stackblitz/alien-signals) 的響應式系統最佳化。\
chibivue 的實作也基於此演算法進行了更新。
:::

## 背景

Vue.js 的響應式系統在 Vue 3.4 中進行了重大效能最佳化。然而，Vue 3.5 切換到了類似 Preact 的 pull-based 演算法，改變了響應式系統的方向。

為了進一步研究 push-pull based 實作，Vue 的核心貢獻者 Johnson Chu 開發了獨立專案 [alien-signals](https://github.com/stackblitz/alien-signals)。

alien-signals 是基於 Vue 3.4 響應式系統重新實作的訊號庫，具有以下特點：
...
alien-signals 基於 Vue 3.4 的響應式系統重新實作，竟然實作了**約 4 倍**的效能提升！\
隨著這項成果被整合到 Vue 3.6 中，所有 Vue 使用者都將受益於這些最佳化。
...
當相依項發生變化時，立即更新所有相依的 computed 值。
...
1. **Push 階段**：當 signal 變化時，在相依的 computed 上設定「dirty」標誌
...
::: warning
本章不會完全移植 alien-signals 的實作，而是解釋其概念和基本機制。\
...
alien-signals 的重要最佳化之一是使用雙向連結串列管理相依關係。

傳統的 Vue 實作使用 Set 管理相依：
...
chibivue 基於 alien-signals 演算法實作響應式系統。

主要檔案：
- `packages/reactivity/dep.ts` - 相依管理
- `packages/reactivity/effect.ts` - effect 實作
- `packages/reactivity/ref.ts` - ref 實作
- `packages/reactivity/computed.ts` - computed 實作
...
後續章節將基於這個最佳化的響應式系統進行建置。

<KawaikoNote variant="base" title="繼續前進">

你理解 alien-signals 的概念了嗎？\
連結串列和版本管理一開始可能感覺很難，但隨著你編寫程式碼，自然會理解的。\
讓我們在下一章中基於這個最佳化的機制實作 ref 和 computed！

</KawaikoNote>

## 總結

- Vue 3.6 將引入基於 alien-signals 的最佳化響應式系統
- Push-Pull 演算法實作高效的 dirty 檢查和延遲評估
- 雙向連結串列的相依管理提高記憶體效率
- 基於版本號的 dirty 檢查避免不必要的重新計算

從下一章開始，我們將在這個最佳化的響應式系統之上實作 ref 和 computed 等 API。
