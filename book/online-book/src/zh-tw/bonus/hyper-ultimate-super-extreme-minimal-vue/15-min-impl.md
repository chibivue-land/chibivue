...
- 掛載
  這是初始渲染。如果 n1 為 null，意味著這是第一次渲染，所以我們需要實作掛載過程。
- 補丁
...
現在讓我們實作邏輯來追蹤在 setup 選項中定義的狀態變化並觸發 render 函式。這個追蹤狀態變化並執行特定操作的過程稱為"響應式系統"。
...
它可以使用 Proxy 物件來實作這一點。代理允許我們為 get/set 操作實作功能。在這種情況下，我們可以使用 set 操作在發生 set 操作時執行補丁過程。
...
讓我們使用之前實作的 render 函式來建立 update 函式：
...
到目前為止，我們已經能夠透過允許使用者使用 render 選項和 h 函式來實作宣告式 UI。但是，實際上，我們希望以類似 HTML 的方式編寫它。

因此，讓我們實作一個模板編譯器，將 HTML 轉換為 h 函式。
...
接下來，讓我們實作從給定字串產生 AST 的 parse 函式。現在，讓我們使用正規表示式和一些字串操作快速實作它。
...
令人驚訝的是，我們能夠在大約 110 行中實作它。（現在沒有人會抱怨了，呼...）
