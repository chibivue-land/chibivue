...
::: info 關於本章
本章介紹如何實作 Vue 的 Scoped CSS 功能。\
學習如何為每個元件隔離樣式，防止樣式衝突。
:::

## 什麼是 Scoped CSS？

Scoped CSS 是將 `<style scoped>` 中定義的樣式僅應用於該元件的功能。
...
此樣式不會影響其他元件中具有相同類名的元素。

<KawaikoNote variant="question" title="為什麼需要 Scoped CSS？">

在大型應用中，不同元件可能使用相同的類名。\
沒有 Scoped CSS，樣式可能會意外影響其他元件。\
透過為每個元件隔離樣式，可以安全地進行樣式設計！
...
<KawaikoNote variant="base" title="嘗試實作！">

參考本章介紹的原理，嘗試自己實作 Scoped CSS！\
...
- Scoped CSS 為每個元件隔離樣式
- 產生唯一的 scopeId 並應用於模板和樣式
- 模板獲得 `data-v-xxx` 屬性，CSS 獲得 `[data-v-xxx]` 選擇器
- 使用 PostCSS 轉換選擇器

## 參考連結

- [Vue.js - Scoped CSS](https://vuejs.org/api/sfc-css-features.html#scoped-css) - Vue 官方檔案
