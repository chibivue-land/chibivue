...
實際上，目前 chibivue 的補丁渲染中存在一個關鍵錯誤。
在實作補丁渲染時，

> 關於 patchChildren，需要透過加入 key 等屬性來處理動態大小的子元素。

你還記得說過這句話嗎？

讓我們看看實際會發生什麼樣的問題。
在目前的實作中，patchChildren 是這樣實作的：
...
實作這些功能的函式是 `patchKeyedChildren`。（讓我們在原始 Vue 中搜尋它。）

方法是首先為新節點產生 key 和索引的映射。
...
首先，讓我們談談如何移動元素。我們在 `nodeOps` 的 `insert` 函式中指定錨點。錨點，顧名思義，是一個錨點，如果你查看在 runtime-dom 中實作的 `insert` 方法，你可以看到它是用 `insertBefore` 方法實作的。
...
首先，讓我們基於 `c2` 產生 `keyToNewIndexMap`（key 到 `c2` 中索引的映射）。
※ 這是之前介紹的程式碼。
...
接下來，讓我們產生 `newIndexToOldIndexMap`。
※ 這是之前介紹的程式碼。
...
```ts
// 執行補丁並產生用於移動的 newIndexToOldIndexMap
...
然後，從獲得的 `newIndexToOldIndexMap` 中取得最長遞增子序列（新實作從這裡開始）。
...
現在我們已經詳細解釋了方法，讓我們實際實作 `patchKeyedChildren`。以下是步驟總結：

1. 準備用於桶接力的 `anchor`（用於在 `move` 中插入）。
2. 基於 `c2` 建立 key 和索引的映射。
3. 基於 key 映射建立 `c2` 和 `c1` 中索引的映射。
