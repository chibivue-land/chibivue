...
因此，我們將實作一個排程器。排程器負責管理任務的執行順序和控制。Vue 排程器的作用之一是在佇列中管理響應式 effect，並在可能的情況下合併它們。
...
至於作業 ID，在這種情況下，我們希望按元件分組，所以我們將為每個元件分配一個唯一識別符（UID）並將其用作作業 ID。
UID 只是透過遞增計數器獲得的識別符。
...
響應式 effect 可以由設定 effect 的一方主動執行，也可以在被加入到相依項（`dep`）後被某些外部操作觸發而被動執行。
對於後一種型別的 effect，它被加入到多個 `depsMap` 並由多個源觸發，需要調度（另一方面，如果它被明確主動呼叫，則不需要這樣的調度）。

讓我們考慮一個具體的例子。在渲染器的 `setupRenderEffect` 函式中，你可能有以下實作：
...
這裡建立的 `effect`，它是一個 `reactiveEffect`，稍後在執行 `setup` 函式時將被響應式物件追蹤。這顯然需要調度的實作（因為它將從各個地方被觸發）。
然而，關於這裡呼叫的 `update()` 函式，它應該簡單地執行 effect，所以不需要調度。
你可能會想，"那我們不能直接呼叫 `componentUpdateFn` 嗎？"但請記住 `run` 函式的實作。簡單地呼叫 `componentUpdateFn` 不會設定 `activeEffect`。
...
在實作方面，除了 `fn` 之外，`ReactiveEffect` 將有一個 `scheduler` 函式，在 `triggerEffect` 函式中，如果存在排程器，將首先執行排程器。
...
function triggerEffect(effect: ReactiveEffect) {
  if (effect.scheduler) {
    ...
    effect.run() // 如果沒有排程器，正常執行 effect
  }
}
```
...
現在，讓我們在閱讀原始碼的同時實作佇列管理調度和 effect 分類！
...
如果你在實作排程器時閱讀了原始碼，你可能已經注意到"nextTick"的出現並想知道它是否在這裡使用。首先，讓我們談談這次我們想要實作的任務。請看這段程式碼：
...
即使我們在更新 `state.count` 後輸出到主控台，資訊也是過時的。這是因為當狀態更新時，DOM 不會立即更新，在主控台輸出時，DOM 仍處於舊狀態。

這就是"nextTick"發揮作用的地方。

https://vuejs.org/api/general.html#nexttick

"nextTick"是排程器的一個 API，它允許你等待直到排程器應用 DOM 更改。"nextTick"的實作非常簡單。它只是保持排程器中正在刷新的作業（promise）並將其連接到"then"。
...
當作業完成時（promise 被解析），傳遞給"nextTick"的回呼被執行。（如果佇列中沒有作業，它連接到"resolvedPromise"的"then"）自然地，"nextTick"本身也回傳一個 Promise，所以作為開發者介面，你可以傳遞回呼或 await "nextTick"。
...
現在，讓我們實際重寫目前排程器的實作以保持"currentFlushPromise"並實作"nextTick"！
