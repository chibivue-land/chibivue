...
在本章中，讓我們為目前無法處理的 Props 實作補丁。
以下是一些需要處理的 Props 示例，但請嘗試透過參考原始實作來實作它們，同時自己填補缺失的部分！
透過這樣做，它應該變得更加實用！

沒有什麼特別新的東西。基於我們到目前為止所做的，應該能夠充分實作它。

我想關注的是 runtime-dom/modules 的實作。
...
要實作這些，需要在基礎模板編譯器部分解釋的 `transform` 概念。
只要不偏離原始 Vue 的設計，它可以在任何地方實作，但我們在這裡跳過它，因為我們想在本書中遵循原始 Vue 的設計。
...
首先，擴充 `patchProp` 的型別定義以接受 `prevChildren` 和 `unmountChildren`。
...
export interface RendererOptions<HostNode = RendererNode, HostElement = RendererElement> {
  patchProp(
    ...
    prevValue: any,
    nextValue: any,
    prevChildren?: VNode<HostNode>[], // 加入
    unmountChildren?: (children: VNode<HostNode>[]) => void, // 加入
  ): void;
  // ...
}
```

接下來，在 `patchDOMProp` 函式中實作 innerHTML/textContent 的處理。
...
然後，在從 `patchProp` 呼叫 `patchDOMProp` 時傳遞 `prevChildren` 和 `unmountChildren`。
...
最後，在 renderer.ts 中呼叫 `hostPatchProp` 時傳遞適當的參數。
