...
Vue 有一個名為插槽的功能，包括三種型別：預設插槽，命名插槽和作用域插槽。
...
機制很簡單。在插槽定義端，我們確保將 slots 作為 setupContext 接收，在使用端用 h 函式渲染元件時，我們只需將渲染函式作為 children 傳遞。
也許對每個人來說最熟悉的用法是在 SFC 的模板中放置一個 slot 元素，但這需要實作一個單獨的模板編譯器，所以我們這次省略它。（我們將在基礎模板編譯器部分介紹它。）

像往常一樣，向實例加入一個可以儲存插槽的屬性，並使用 createSetupContext 將其作為 SetupContext 混合。
修改 h 函式，使其可以接收渲染函式作為第三個參數，而不僅僅是陣列，如果傳遞了渲染函式，在產生實例時將其設定為元件實例的預設插槽。
讓我們先實作到這一點！

（由於在 children 中實作了 normalize，ShapeFlags 已經略有更改。）
...
當然，我們將在基礎模板編譯器部分實作一個可以處理作用域插槽的編譯器，但它們將被轉換為這些形式。
