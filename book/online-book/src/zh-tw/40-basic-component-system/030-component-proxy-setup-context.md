...
元件有一個重要概念叫做代理（Proxy）。
簡單來說，它是一個允許存取元件實例資料（公開屬性）的代理。
代理結合了 setup 的結果（狀態，函式），data，props 和其他存取。

讓我們考慮以下程式碼（包括在 chibivue 中未實作的部分，所以請將其視為常規 Vue）：
...
在這種情況下，你可以透過 ref 存取元件的資訊。

為了實作這一點，ComponentInternalInstance 有一個名為 proxy 的屬性，它儲存用於資料存取的代理。
...
export const PublicInstanceProxyHandlers: ProxyHandler<any> = {
  get(instance: ComponentRenderContext, key: string) {
    ...
    // 根據鍵按 setupState -> props -> ctx 的順序檢查，如果存在則回傳值
  },
}
```
...
一旦實作，讓我們修改程式碼以將此代理傳遞給渲染函式和 ref。
...
※ 順便說一下，我還實作了 defineComponent 和相關型別檢查的實作（這允許我們推斷代理資料的型別）。
...
export interface ComponentInternalInstance {
  ...
  // .
  // .
  setupContext: SetupContext | null // 加入
}
...
然後，在建立實例時，產生 setupContext 並在執行 setup 函式時將此物件作為第二個參數傳遞。

## expose

一旦你到達這一點，讓我們嘗試實作除 emit 之外的 SetupContext。
作為這次的例子，讓我們實作 expose。

expose 是一個允許你明確定義公開屬性的函式。
讓我們目標實作如下的開發者介面：
...
對於不使用 expose 的元件，預設情況下一切仍然是公開的。

作為方向，讓我們在實例內部有一個名為 `exposed` 的物件，如果這裡設定了值，我們將把這個物件傳遞給 templateRef 的 ref。
...
export interface ComponentInternalInstance {
  ...
  // .
  // .
  exposed: Record<string, any> | null // 加入
}
```

讓我們實作 expose 函式，以便可以在這裡註冊物件。

## ProxyRefs

在本章中，我們實作了 proxy 和 exposedProxy，但實際上與原始 Vue 有一些差異。
那就是"ref 被解包"。（在 proxy 的情況下，setupState 具有此屬性而不是 proxy。）

這些是用 ProxyRefs 實作的，處理器在名為 `shallowUnwrapHandlers` 的名稱下實作。
...
[使用 with 語句和代理建立動態命名空間 (MDN)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/with#creating_dynamic_namespaces_using_the_with_statement_and_a_proxy)

所以讓我們在 PublicInstanceProxyHandlers 中實作 has。
如果鍵存在於 setupState，props 或 ctx 中，它應該回傳 true。
