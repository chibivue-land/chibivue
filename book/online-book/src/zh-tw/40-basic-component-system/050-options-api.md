...
到目前為止，應該不會太困難。這是可以應用於 "defineComponent" 參數的型別。
當然，在 "defineComponent" 中，我們也接受 "D" 和 "M" 來中繼使用者定義的型別。這允許我們中繼使用者定義的型別。
...
一旦我們有了這個，我們就將實例型別混合到 "ComponentOptions" 中的 "this" 中。
...
在實踐中，我們需要推斷各種型別，如 "props"，"computed" 和 "inject"，但基本原理是相同的。
乍一看，你可能會被許多泛型和型別轉換（如從 "inject" 中僅提取 "key"）所壓倒，但如果你冷靜下來並回到原理，你應該沒問題。
在本書的程式碼中，受原始 Vue 的啟發，我們用 "CreateComponentPublicInstance" 進一步抽象了一步，並實作了一個名為 "ComponentPublicInstanceConstructor" 的型別，但不要太擔心。（如果你感興趣，你也可以閱讀它！）
