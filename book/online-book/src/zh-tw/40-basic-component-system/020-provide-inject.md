...
## 讓我們實作 Provide/Inject

這是 Provide 和 Inject 的實作。實作也相當簡單。
基本概念是在 ComponentInternalInstance 中有一個地方來儲存提供的資料（provides），並保持父元件的實例來繼承資料。

需要注意的一點是，provide 有兩個入口點。一個是在元件的 setup 期間，這很容易想像，
...
const app = createApp({
  setup() {
    ...
    //.
    //.
    provide('key', someValue) // 這是從元件呼叫 provide 的情況
    //.
    //.
  },
})
...
現在，我們應該在哪裡儲存 app 提供的內容？由於 app 不是元件，這是一個問題。

為了給你答案，讓我們說 app 實例有一個名為 AppContext 的物件，我們將在其中儲存 provides 物件。

將來，我們將向這個 AppContext 加入全域元件和自訂指令設定。

現在我們已經解釋了到目前為止的一切，讓我們實作程式碼，使其按以下方式工作！
