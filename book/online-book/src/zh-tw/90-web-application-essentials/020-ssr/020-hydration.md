...
在上一章中，我們學習了如何使用 `renderToString` 將 Vue 元件渲染為 HTML 字串。但是，SSR 產生的 HTML 只是靜態標記——事件處理器和響應式都不起作用。

Hydration（水合）是將伺服器產生的 HTML「激活」為使用者端 Vue 應用程式的過程。
...
- `patchProp`：將屬性（特別是事件處理器）附加到 DOM 元素的函式
- `nextSibling`：遍歷 DOM 樹的函式

## createHydrationRenderer 實作
...
`hydrate` 函式從容器的第一個子節點開始，並行遍歷 VNode 樹和 DOM 樹。

### hydrateNode - 根據節點型別分支
...
按順序處理 VNode 子元素和 DOM 子節點。每個 `hydrateNode` 回傳下一個兄弟節點，用於繼續遍歷。
...
在 Hydration 期間，SSR 產生的 HTML 必須與使用者端產生的 VNode 匹配。如果不匹配，就會發生「Hydration 不匹配」。

### 常見原因

1. **日期/亂數**：`new Date()` 或 `Math.random()` 在伺服器和使用者端產生不同的值
2. **瀏覽器特定的 API**：`window` 或 `localStorage` 在伺服器上不存在
3. **條件分支**：伺服器和使用者端走不同的程式碼路徑
...
目前實作是最小化的，但 Vue 本身有以下功能：

1. **Hydration 不匹配偵測**：在開發模式下偵測伺服器/使用者端不一致
2. **部分 Hydration**：只水合必要的部分（效能最佳化）
3. **使用 PatchFlags 最佳化**：跳過靜態節點的 Hydration
4. **非同步元件 Hydration**：與 `Suspense` 整合

<KawaikoNote variant="surprise" title="Hydration 完成！">

現在我們擁有了 SSR 的所有部分。
透過使用 `renderToString` 進行伺服器端渲染和
