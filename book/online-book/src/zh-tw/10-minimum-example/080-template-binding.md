...
現在，我想能夠在模板中處理從 `setup` 函式回傳的值。\
從現在開始，我將把這稱為「模板綁定」或簡稱「綁定」。\
我將實作綁定，但在實作事件處理程式和 mustache 語法之前，有幾件事我想做。

我提到了從 `setup` 回傳的值，但目前 `setup` 的回傳值要麼是 `undefined`，要麼是一個函式（渲染函式）。\
作為實作綁定的準備，我需要修改它，使 `setup` 可以回傳狀態和其他值，並且這些值可以作為元件資料儲存。
...
現在，在實作編譯器之前，讓我們思考如何將 `setupState` 綁定到模板。\
...
我們不知道 Vue.js 的實作將來會如何變化，但由於 Vue.js 3 使用 `with` 語句，我們將在此實作中使用它。

稍微說一下，Vue.js 中並非所有內容都使用 `with` 語句實作。\
在處理單檔案元件（SFC）中的模板時，它是在不使用 `with` 語句的情況下實作的。\
我們將在後面的章節中介紹這一點，但現在，讓我們考慮使用 `with` 來實作它。
...
首先，讓我們實作 Mustache 語法。\
像往常一樣，我們將考慮 AST，實作解析器，然後實作程式碼產生器。\
...
export type TemplateChildNode = ElementNode | TextNode | InterpolationNode // 加入 InterpolationNode
...
對於那些到目前為止已經實作了解析器的人來說，應該沒有特別困難的部分。\
它只是搜尋 <span v-pre>`{{`</span> 並讀取直到 <span v-pre>`}}`</span> 出現，產生 AST。\
如果沒有找到 <span v-pre>`}}`</span>，它回傳 undefined 並在 parseText 的分支中將其解析為文字。

讓我們輸出到主控台或其他地方，以確保解析正常工作。
...
我想這樣說，但實作還不夠乾淨，所以我想稍微重構一下。\
由於 `@click` 被歸類為「指令」名稱，很容易想像將來實作 `v-bind` 和 `v-model`。\
所以讓我們在 AST 中將其表示為 `DIRECTIVE` 並將其與簡單的 `ATTRIBUTE` 區分開來。

像往常一樣，讓我們按照 AST -> parse -> codegen 的順序實作它。
