...
所以，從這裡開始，我將認真實作一個解析器。

## AST 的介紹

為了實作一個成熟的編譯器，我將引入一個叫做 AST（抽象語法樹）的東西。\
AST 代表抽象語法樹，顧名思義，它是表示語法的樹結構的資料表示。\
這是在實作各種編譯器時出現的概念，不僅僅是 Vue.js。\
...
我試圖以一種酷的方式解釋它，但在圖像方面，它只是我們迄今為止實作的 parse 函式回傳型別的正式定義。\
目前，parse 函式的回傳值如下：
...
讓我們擴充這個並定義它，以便可以執行更複雜的運算式。

建立一個新檔案 `~/packages/compiler-core/ast.ts`。\
我將在編寫程式碼時解釋，因為它有點長。
...
// loc 代表位置，儲存關於這個 Node 在原始碼（模板字串）中對應位置的資訊。
// （例如，哪一行和行上的哪個位置）
...
這是我們這次將要處理的 AST。\
在 parse 函式中，我們將實作將模板字串轉換為這個 AST。
...
請注意，這本線上書籍參考的是此重寫之前的實作。
我們計劃在適當的時機相應地更新這本線上書籍。
:::

在 `~/packages/compiler-core/parse.ts` 中實作它。
即使我說它是成熟的，你也不必太緊張。\
基本上，你所做的就是在讀取字串時產生 AST，並使用分支和迴圈。\
原始碼會有點長，但我認為在程式碼庫中解釋會更容易理解。所以讓我們這樣進行。\
請透過閱讀原始碼來嘗試理解細節。

刪除您迄今為止實作的 baseParse 的內容，並按如下方式更改回傳型別：
...
export const baseParse = (
  ...
  const context = createParserContext(content) // 建立上下文
...
雖然有點長，但讓我們從 parseChildren 的實作開始。\
解釋將在原始碼的註解中完成。
...
function parseChildren(
  ...
  while (!isEnd(context, ancestors)) {
    ...
    if (s[0] === '<') {
      ...
      if (/[a-z]/i.test(s[1])) {
        node = parseElement(context, ancestors) // TODO: 稍後實作這個。
      }
    }

    if (!node) {
      // 如果不匹配上述條件，則將其解析為 TextNode。
      node = parseText(context) // TODO: 稍後實作這個。
    }
...
接下來，讓我們實作 parseElement 和 parseText。
...
雖然這部分程式碼在解析器的最近重寫中被消除了，但將 Vue 3.3 程式碼修改為只檢查 ancestors 中的最後一個元素仍然會導致所有正面測試成功透過。
:::

## parseText

首先，讓我們從簡單的 parseText 開始。\
它有點長，因為它還實作了一些不僅在 parseText 中使用，而且在其他函式中也使用的實用程式。
...
開始標籤的解析進一步分為標籤名和屬性。\
讓我們首先建立一個框架來解析開始標籤的前半部分，子節點和結束標籤。
...
我認為在這裡閱讀實作比用特殊詞彙解釋更好，所以請反覆閱讀。\
雖然我寫了很多，但基本上它是透過讀取字串推進分析的直接任務，沒有特別困難的技術。

到現在，您應該能夠產生 AST。讓我們檢查解析是否正常工作。\
但是，由於 codegen 部分尚未實作，我們這次將輸出到主控台進行確認。
