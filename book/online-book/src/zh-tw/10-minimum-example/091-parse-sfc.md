...
雖然這是我們之前建立的範例外掛程式，但讓我們刪除它，因為它不再需要了。
...
另外，為了建立 Vite 外掛程式，請安裝主要的 Vite 套件。
...
這是外掛程式的主要部分，但由於這原本超出了 vuejs/core 的範圍，我們將在 `packages` 目錄中建立一個名為 `@extensions` 的目錄並在那裡實作它。
...
現在，讓我們實作 SFC 編譯器。\
但是，沒有任何實質內容可能很難想像，所以讓我們實作一個遊樂場並在執行時進行。\
我們將建立一個簡單的 SFC 並載入它。
...
我們可以像在範例中那樣使用 `id` 編寫分支語句，但由於 Vite 提供了一個名為 `createFilter` 的函式，讓我們使用它建立一個過濾器。\
（這沒有特別的原因。）
...
我們建立了一個過濾器，如果是 Vue 檔案，則將檔案內容轉換為 `export default {}`。\
錯誤應該消失，螢幕應該不顯示任何內容。

## 在 compiler-sfc 上實作解析器

現在，這只是一個臨時解決方案，所以讓我們實作一個合適的解決方案。\
...
你們中的一些人可能在想，「什麼？你在模板解析器上如此努力工作，現在你要建立另一個解析器...？這很麻煩。」但不要擔心。\
我們在這裡要實作的解析器並不是什麼大事。那是因為我們只是透過結合我們迄今為止建立的內容來分離模板，腳本和樣式。

首先，作為準備，匯出我們之前建立的模板解析器。
...
我認為對於到目前為止已經實作了解析器的每個人來說都很容易。讓我們在外掛程式中實際解析 SFC。
...
這段程式碼在 Vite 執行的程式中執行，這意味著它在 Node 中執行，所以我認為主控台輸出會顯示在終端中。
