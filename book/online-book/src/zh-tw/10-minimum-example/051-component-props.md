...
我們需要指定為 `props: { message: { type: String } }` 的 props 定義，以及一個實際儲存 props 值的屬性，所以我們加入以下內容：
...
export interface ComponentInternalInstance {
  ...
  // .
  // .
  propsOptions: Props // 儲存像 `props: { message: { type: String } }` 這樣的物件

  props: Data // 儲存從父元件傳遞的實際資料（在這種情況下，它將是像 `{ message: "hello" }` 這樣的東西）
}
```

建立一個名為 `~/packages/runtime-core/componentProps.ts` 的新檔案，內容如下：
...
export type ComponentOptions = {
  props?: Record<string, any> // 加入
  setup?: () => Function
  render?: Function
}
```

當使用 `createComponentInstance` 產生實例時，在產生實例時將 propsOptions 設定到實例中。
...
在元件掛載時，根據 propsOptions 過濾 vnode 持有的 props。\
使用 `reactive` 函式將過濾後的物件轉換為響應式物件，並將其分配給 `instance.props`。

在 `componentProps.ts` 中實作一個名為 `initProps` 的函式來執行這一系列步驟。
...
此時，props 應該傳遞給子元件，所以讓我們在遊樂場中檢查它。
...
要使此元件工作，我們需要在 `componentProps.ts` 中實作 `updateProps` 並在元件更新時執行它。
...
讓我們整理一下元件更新處理的流程。\
當父元件重新渲染時，傳遞給子元件的 props 可能會改變。\
流程如下：

1. 父元件的 `render` 函式被執行，為子元件產生新的 VNode
2. 在 `patch` 處理中，`processComponent` 被呼叫，比較現有元件（`n1`）和新的 VNode（`n2`）
3. 如果存在現有元件，則呼叫 `updateComponent` 函式

首先，在 `ComponentInternalInstance` 中加入 `next` 屬性。

```ts
export interface ComponentInternalInstance {
  // .
  // .
  vnode: VNode // 目前的VNode
  next: VNode | null // 當有來自父元件的更新請求時，新的VNode會被設定在這裡
  // .
  // .
}
```

接下來，在 `processComponent` 中實作已掛載元件的更新處理。
...
const updateComponent = (n1: VNode, n2: VNode) => {
  ...
  instance.update(); // 觸發元件更新
};
```

在 `updateComponent` 中，我們將新的 VNode（`n2`）設定到 `instance.next`，然後呼叫 `instance.update()`。\
...
const setupRenderEffect = (
    ...
    const componentUpdateFn = () => {
      ...
      if (!instance.isMounted) {
        ...
        if (next) {
          // 當有來自父元件的更新請求時（例如，props 改變了）
          next.el = vnode.el; // 將目前DOM元素引用繼承到新VNode
          next.component = instance; // 將實例引用設定到新VNode
          instance.vnode = next; // 將實例的「目前VNode」切換為新的
          instance.next = null; // 已處理完畢，重置為null
          updateProps(instance, next.props); // 用新的props更新實例的props
        }
        // 如果next不存在，則是由於元件自身響應式狀態變化而導致的重新渲染
```

當 `instance.next` 存在時，意味著有來自父元件的更新請求（如 props 改變）。\
在這種情況下，我們先將新 VNode 的資訊反映到實例中，然後再更新 props。\
當 `instance.next` 不存在時，則是由於元件自身內部狀態（響應式值）的變化而導致的重新渲染。

如果螢幕更新了，那就沒問題。\
現在，您可以使用 props 將資料傳遞給元件！做得很好！
...
作為附註，雖然這不是必需的，但讓我們實作接收 kebab-case props 的能力，就像原始 Vue 中一樣。\
此時，建立一個名為 `~/packages/shared` 的目錄，並在其中建立一個名為 `general.ts` 的檔案。\
這是定義通用函式的地方，不僅適用於 `runtime-core` 和 `runtime-dom`。\
按照原始 Vue，讓我們實作 `hasOwn` 和 `camelize`。
