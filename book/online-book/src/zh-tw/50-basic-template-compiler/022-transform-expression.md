...
- 問題 1：註冊為事件處理器的 `increment` 無法存取 `_ctx`。
  這是因為在之前的 `v-bind` 實作中沒有加入前綴。
- 問題 2：運算式 `count + count` 無法存取 `_ctx`。
  關於 mustache 語法，它只在開頭加入 `_ctx.`，無法處理其他識別符。
  因此，運算式中出現的所有識別符都需要加上 `_ctx.` 前綴。這適用於所有部分，不僅僅是 mustache。

看起來需要一個過程來為運算式中出現的識別符加入 `_ctx.`。
...
如下所示，在原始實作中，從 `setup` 函式綁定的任何內容都透過 `$setup` 解析。

![Original resolve bindings output](/figures/50-basic-template-compiler/transform-expression/resolve-bindings-original.png)

然而，實作這個有點困難，所以我們將簡化它並透過加入 `_ctx.` 來實作。（所有 props 和 setup 都將從 `_ctx` 解析）

:::

## 實作方法

簡單來說，我們想要做的是"在 ExpressionNode 上的每個識別符（名稱）的開頭加入 `_ctx.`"。

讓我更詳細地解釋一下。
作為回顧，程式透過解析被表示為 AST。
表示程式的 AST 主要有兩種型別的節點：Expression 和 Statement。
這些通常被稱為運算式和語句。
...
有各種型別的運算式。Identifier 是其中之一，它是由識別符表示的運算式。
（你可以將其視為一般的變數名）

Identifier 出現在運算式的各個地方。
...
建立一個名為 `compiler-core/babelUtils.ts` 的新檔案，並實作可以對 Identifier 執行操作的實用函式。
...
然後，為運算式產生 AST 並將其傳遞給此函式，在重寫節點的同時執行轉換。

## transformExpression 的實作

### InterpolationNode 的 AST 和解析器更改

我們將實作轉換過程的主體 transformExpression。
...
為了使運算式轉換在其他轉換器中可用，我們將其提取為名為 `processExpression` 的函式。
...
接下來，讓我們解釋 processExpression 的實作。
首先，我們將實作一個名為 rewriteIdentifier 的函式來重寫 node 內的 Identifier。
如果 node 是單個 Identifier，我們簡單地應用此函式並回傳它。

需要注意的一點是，這個 processExpression 特定於 SFC（單檔案元件）情況（不使用 with 語句的情況）。
換句話說，如果設定了 isBrowser 標誌，我們實作它簡單地回傳 node。
我們修改實作以透過 ctx 接收標誌。

另外，我想保留像 true 和 false 這樣的字面值，所以我將為字面值建立一個白名單。
...
`makeMap` 是在 vuejs/core 中實作的用於存在性檢查的輔助函式，它回傳一個布林值，指示是否與用逗號分隔定義的字串匹配。
...
在以下討論中，請注意我們將處理兩個不同的 AST：Babel 產生的 JavaScript AST 和 chibivue 定義的 AST。
...
首先，讓我們從階段 1 開始。
這相對簡單。如果我們可以用 Babel 解析原始 SimpleExpressionNode 內容（字串）並獲得 estree，我們可以透過我們之前建立的實用函式傳遞它並應用 rewriteIdentifier。
此時，我們收集 estree 節點。
...
以下運算式將被解析為以下 CompoundExpressionNode：
...
這是一種相當奇怪的感覺。"children" 採用字串型別的原因是因為它採用這種形式。
...
換句話說，我們要做的是基於收集的 estree 的 Identifier Node 和源產生這個 CompoundExpression。
以下程式碼是為此的實作。
...
請仔細查看原始碼了解更多詳細資訊。如果你理解到目前為止的策略，你應該能夠閱讀它。（另外，請查看 advancePositionWithClone 等的實作，因為它們是新實作的。）

現在我們可以產生 CompoundExpressionNode，讓我們也在 Codegen 中支援它。
...
現在我們已經實作到這裡，讓我們完成編譯器並嘗試執行它！

```ts
// 加入 transformExpression
