...
到目前為止，我們已經實作了 v-bind，v-on，v-if，v-for，v-model 等主要指令。\
在本章中，我們將實作其餘的內建指令。
...
### 在 compiler-dom 中實作 transformer

由於 v-text 是 DOM 特有的指令，我們在 compiler-dom 中實作它。
...
雖然可以使用現有的 `hostPatchProp` 來實作 `hostRemoveAttribute`，但讓我們簡單地將其新增到 `nodeOps` 中。
