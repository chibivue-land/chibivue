# 轉換器和程式碼產生重構的實作（基礎模板編譯器部門開始）

## 現有實作的回顧

現在，讓我們從最小示例部門停下的地方開始更認真地實作模板編譯器。距離我們上次處理它已經有一段時間了，所以讓我們回顧一下目前的實作。主要關鍵詞是 Parse，AST 和 Codegen。
...
Vue.js 的模板編譯器分為兩部分：表示解析模板結果的 AST 和表示產生程式碼的 AST。
我們目前的實作只處理前一個 AST。

讓我們考慮將模板 `<p>hello</p>` 作為輸入的情況。

首先，透過解析產生以下 AST。這與現有實作相同。
...
至於"表示產生程式碼的 AST"，讓我們考慮應該產生什麼樣的程式碼。
我認為應該是這樣的：
...
這是表示產生的 JavaScript 程式碼的 AST。
換句話說，它是一個表示用於產生應該產生的程式碼的 AST 的物件。
...
透過這種方式，表示由 Codegen 產生的程式碼的 AST 被表達。
你可能在這一點上感覺不到分離它們的必要性，但在將來實作指令時會很有用。
透過分離專注於輸入的 AST 和專注於輸出的 AST，我們可以使用稱為 `transform` 的函式執行從 `input AST -> output AST` 的轉換。

## Codegen 節點

現在我們已經掌握了流程，讓我們確認我們將處理什麼樣的節點（我們想要轉換什麼樣的節點）。我將在枚舉它們並提供註釋的同時進行解釋。請參考原始碼取得準確資訊，因為某些部分被省略了。
...
export interface VNodeCall extends Node {
  ...
  tag: string | symbol
  props: ObjectExpression | undefined // 注意：在原始碼中實作為 PropsExpression（用於未來擴充）
  children:
    | TemplateChildNode[] // multiple children
...
在實作轉換器之前，讓我們談談設計。首先，重要的是要注意有兩種型別的轉換器：NodeTransform 和 DirectiveTransform。這些分別用於轉換節點和指令，並採用以下介面。
...
DirectiveTransform 將在實作指令時在後面的章節中介紹，所以現在讓我們稱之為 Function。
NodeTransform 和 DirectiveTransform 實際上都是函式。你可以將它們視為轉換 AST 的函式。
請注意，NodeTransform 的結果是一個函式。在實作 transform 時，如果你實作它回傳一個函式，該函式將在該節點的轉換之後執行（它被稱為 onExit 過程）。
你想在節點的 transform 之後執行的任何處理都應該在這裡描述。我將在稍後描述稱為 traverseNode 的函式時解釋這一點。
介面的解釋主要如上所述。

作為更具體的實作，有用於轉換元素的 transformElement 和用於轉換運算式的 transformExpression 等。
至於 DirectiveTransform 的實作，每個指令都有實作。
這些實作在 compiler-core/src/transforms 中實作。具體的轉換過程在這裡實作。
...
接下來，關於上下文，TransformContext 儲存在這些轉換期間使用的資訊和函式。
將來會加入更多，但現在這就足夠了。
...
結構非常簡單，只需產生上下文並使用 traverseNode 函式遍歷節點。
這個 traverseNode 函式是轉換的主要實作。
...
在 traverseNode 中，基本上，它只是將儲存在上下文中的 nodeTransforms（轉換節點的函式集合）應用於節點。
對於那些有子節點的，子節點也透過 traverseNode 傳遞。
在介面解釋期間提到的 onExit 的實作也在這裡。
...
接下來，讓我們談談具體的轉換過程。作為示例，讓我們實作 transformElement。

在 transformElement 中，我們主要將型別為 NodeTypes.ELEMENT 的節點轉換為 VNodeCall。
...
這是一個簡單的物件到物件的轉換，所以我認為不會很困難。讓我們嘗試透過閱讀原始碼來實作它。
...
現有的 Codegen 實作非常簡單，所以讓我們在這裡使它更正式一些（因為它相當硬編碼）。
讓我們也建立一個 Codegen 特定的上下文並將產生的程式碼推送到其中。
此外，讓我們在上下文中實作一些輔助函式（如縮排）。
...
我將在這裡省略實作細節，但我只是為每個角色分離了函式，實作方法沒有重大變化。
由於我還沒有能夠支援指令，由於在該區域刪除了臨時實作，有些部分不工作，但
如果程式碼大致按以下方式工作，就可以了！
