...
在繼續實作之前，讓我們進行一些重構。
目前，在 codegen 產生的程式碼中，我們從 `shared` 和 `runtime-core` 匯入（或解構）了許多輔助函式。
而且在 codegen（和 transform）的實作中，我們硬編碼了函式名稱。這不是很明智。

這次，讓我們將它們重構為 `runtime-helper` 並用符號集中管理，進一步地，更改實作以僅匯入必要的內容。

首先，讓我們在 `compiler-core/runtimeHelpers.ts` 中實作表示每個輔助函式的符號。
到目前為止，我們一直使用 `h` 函式來產生 VNode，但這次，讓我們按照原始實作更改為使用 `createVNode`。
...
在 `TransformContext` 中實作一個區域來註冊輔助函式和註冊它們的函式。
...
export function transform(root: RootNode, options: TransformOptions) {
  ...
  root.helpers = new Set([...context.helpers.keys()]) // 將輔助函式加入到 root
}
```

```ts
// 根據原始實作加入 `_` 作為前綴來為其設定別名
const aliasHelper = (s: symbol) => `${helperNameMap[s]}: _${helperNameMap[s]}`

function genFunctionPreamble(ast: RootNode, context: CodegenContext) {
  const { push, newline, runtimeGlobalName } = context

  // 基於在 ast 中註冊的輔助函式產生輔助函式宣告
...
實際上，關於解析器的實作，前一章的實作就足夠了，問題在於轉換器的實作。
轉換的內容主要根據 arg 的存在與否和 exp 的型別而變化。
當沒有 arg 時，需要做的事情幾乎與 v-bind 相同。

換句話說，需要考慮的是可以作為 arg 的 exp 型別以及為它們轉換必要的 AST Node。
...
暫時，讓我們實作沒有 arg 的情況，因為它與 v-bind 相同。
...
首先，這些格式可以大致分為兩類："函式"和"語句"。在 Vue 中，如果是單個 Identifier，單個 MemberExpression 或函式運算式，則將其視為函式。否則，它是一個語句。在原始碼中，它似乎被稱為 inlineStatement。
...
1. 首先，確定它是否是函式（單個 Identifier 或單個 MemberExpression 或函式運算式）。

2-1. 如果是函式，產生 `eventName: exp` 形式的 ObjectProperty，不進行任何轉換。

2-2. 如果不是函式（如果是 inlineStatement），將其轉換為 `$event => { ${exp} }` 的形式並產生 ObjectProperty。

這就是基本思路。

#### 確定是函式運算式還是語句

讓我們從實作確定開始。是否是函式運算式是使用正規運算式完成的。
...
是否是單個 Identifier 或單個 MemberExpression 是用名為 `isMemberExpression` 的函式實作的。

```ts
const isMemberExp = isMemberExpression(exp.content)
```

這個 `isMemberExpression` 函式相當複雜，實作很長。有點長，所以我在這裡省略它。（如果你感興趣，請查看程式碼。）
...
現在，我們已經完成了這次的關鍵部分。讓我們實作剩餘的必要部分並完成 v-on！！
