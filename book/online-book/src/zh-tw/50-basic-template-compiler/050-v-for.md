...
現在，讓我們繼續指令的實作。這次，讓我們嘗試支援 v-for。
...
你可能會想，"我們突然要實作這麼多東西？這不可能！"但不要擔心，我會一步一步地解釋。

## 實作方法

首先，讓我們大致思考一下我們想要如何編譯它，並考慮在實作時可能遇到的困難點。

首先，讓我們看看期望的編譯結果。

基本結構並不那麼困難。我們將在 runtime-core 中實作一個名為 renderList 的輔助函式來渲染列表，並將其編譯為運算式。
...
將來，作為 renderList 第一個參數傳遞的值預期不僅是陣列，還可能是數字和物件。但是，現在讓我們假設只期望陣列。\_renderList 函式本身的實作可以理解為類似於 Array.prototype.map 的東西。至於除陣列之外的值，你只需要在 \_renderList 中對它們進行正規化，所以現在讓我們忘記它們（只關注陣列）。

現在，對於那些到目前為止已經實作了各種指令的人來說，實作這種編譯器（轉換器）應該不會太困難。

## 關鍵實作點（困難點）

困難點在於在 SFC（單檔案元件）中使用它時。你還記得在 SFC 中使用的編譯器和在瀏覽器中使用的編譯器之間的區別嗎？是的，就是使用 `_ctx` 解析運算式。
...
現在，讓我們暫時擱置如何實作這一點，從大局開始實作。

## AST 的實作

現在，讓我們像往常一樣定義 AST。

與 v-if 一樣，我們將考慮轉換後的 AST（無需實作解析器）。
...
// 還支援函式運算式，因為回呼函式用作 renderList 的第二個參數。
...
// 在 v-for 的情況下，回傳是固定的，所以它被表示為專門用於此目的的 AST。
...
關於 `RENDER_LIST`，像往常一樣，將其加入到 `runtimeHelpers`。
...
我們收集由 `value`，`key` 和 `index` 定義的識別符，並跳過加入前綴。

## codegen 的實作

雖然順序有點顛倒，但讓我們先實作 codegen，因為沒有太多要討論的。
只需要做兩件事：處理 `NodeTypes.FOR` 和函式運算式的 codegen（這是第一次出現）。
...
現在，在我們繼續主要實作之前，讓我們思考如何收集識別符。

這次，我們需要考慮不僅是像 `fruit` 這樣的簡單識別符，還有像 `{ id, name, color }` 這樣的解構賦值。
為此，似乎我們需要像往常一樣使用 TreeWalker。

目前，在 `processExpression` 函式中，實作是搜尋識別符並向它們加入 `_ctx`。但是，這次我們只需要收集識別符而不加入任何東西。讓我們實作這一點。

首先，讓我們準備一個地方來儲存收集的識別符。由於如果每個 Node 都有它們對於 codegen 和其他目的會很方便，讓我們向 AST 加入一個可以在每個 Node 上儲存多個識別符的屬性。

目標是 `CompoundExpressionNode` 和 `SimpleExpressionNode`。

像 `fruit` 這樣的簡單識別符將被加入到 `SimpleExpressionNode`，
像 `{ id, name, color }` 這樣的解構賦值將被加入到 `CompoundExpressionNode`。（在視覺化方面，它將是一個複合運算式，如 `["{", simpleExpr("id"), ",", simpleExpr("name"), ",", simpleExpr("color"), "}"]`）
...
在 `processExpression` 函式中，讓我們在這裡實作收集識別符的邏輯，並透過將收集的識別符加入到轉換器的上下文中來跳過加入前綴。

目前，用於加入/刪除識別符的函式被設定為接收單個識別符作為字串，所以讓我們將其更改為假設 `{ identifier: string[] }` 的形式。
...
現在，讓我們在 `processExpression` 函式中實作收集識別符的邏輯。

在 `processExpression` 函式中，定義一個名為 `asParams` 的選項，如果設定為 true，實作跳過加入前綴並在 `node.identifiers` 中收集識別符的邏輯。
...
為此，我們將使用在 `babelUtils` 中實作的 `walkIdentifiers`。
...
在 `walkIdentifiers` 中收集後，最後，在產生 CompoundExpression 時基於 `knownIds` 產生識別符。
...
ret.identifiers = Object.keys(knownIds) // 基於 knownIds 產生識別符
return ret
```

雖然檔案有點亂序，但 `walkFunctionParams` 和 `markScopeIdentifier` 只是遍歷參數並將 `Node.name` 加入到 `knownIds`。
...
有了這個，我們應該能夠收集識別符。讓我們使用這個實作 `transformFor` 並完成 v-for 指令！

### transformFor

現在我們已經克服了障礙，讓我們像往常一樣使用我們擁有的東西實作轉換器。
還有一點點，讓我們加油！

像 v-if 一樣，這也涉及結構，所以讓我們使用 `createStructuralDirectiveTransform` 來實作它。

我認為如果我用程式碼寫解釋會更容易理解，所以我將在下面提供帶有解釋的程式碼。但是，請在查看這個之前嘗試透過閱讀原始碼自己實作它！

```ts
// 這是主要結構的實作，類似於 v-if。
// 它在適當的地方執行 processFor 並在適當的地方產生 codegenNode。
// processFor 是最複雜的實作。
export const transformFor = createStructuralDirectiveTransform(
  'for',
  (node, dir, context) => {
    return processFor(node, dir, context, forNode => {
      // 如預期的那樣，產生呼叫 renderList 的程式碼。
      const renderExp = createCallExpression(context.helper(RENDER_LIST), [
        forNode.source,
      ]) as ForRenderListExpression

      // 為作為 v-for 容器的 Fragment 產生 codegenNode。
...
export function parseForExpression(
  ...
  if (iteratorMatch) {
    ...
    if (keyContent) {
      ...
      if (!context.isBrowser) {
        // 如果不在瀏覽器模式下，將 asParams 設定為 true 並收集 key 的識別符。
    ...
    if (iteratorMatch[2]) {
      ...
      if (indexContent) {
        ...
        if (!context.isBrowser) {
          // 如果不在瀏覽器模式下，將 asParams 設定為 true 並收集 index 的識別符。
  ...
  if (valueContent) {
    ...
    if (!context.isBrowser) {
      // 如果不在瀏覽器模式下，將 asParams 設定為 true 並收集 value 的識別符。
...
現在，剩下的部分是實際包含在編譯程式碼中的 renderList 的實作，以及註冊轉換器的實作。如果我們能實作這些，v-for 應該就能工作了！
