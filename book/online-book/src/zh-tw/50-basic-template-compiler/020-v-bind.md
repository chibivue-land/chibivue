...
現在讓我們實作指令，這是 Vue.js 的精髓。
像往常一樣，我們將指令應用到轉換器，出現在那裡的介面稱為 DirectiveTransform。
DirectiveTransform 接受 DirectiveNode 和 ElementNode 作為參數，並回傳轉換後的 Property。
...
我們將更新解析器實作以遵循這個 AST 修改。我們將 exp 和 arg 解析為 SimpleExpressionNode。

我們還將解析 v-on 中使用的 @ 和插槽中使用的 #。
（由於考慮正規運算式很麻煩（而且在解釋時逐漸加入它們很麻煩），我們現在將借用原始程式碼。）
...
function parseAttribute(
  ...
  if (/^(v-[A-Za-z0-9-]|:|\.|@|#)/.test(name)) {
    ...
    if (match[2]) {
      ...
      // 如果是像 "[arg]" 這樣的動態參數，將 isStatic 設定為 false 並提取內容作為內容
...
接下來，讓我們編寫將此 AST 轉換為 Codegen AST 的實作。
...
由於我們這次想要實作 v-bind，我們將實作一個名為 transformVBind 的函式，
...
另外，對於 class 和 style，它們有各種開發者介面，所以需要進行正規化。
https://vuejs.org/api/built-in-directives.html#v-bind

實作名為 normalizeClass 和 normalizeStyle 的函式，並分別應用它們。

如果 arg 是動態的，無法確定具體的，所以實作一個名為 normalizeProps 的函式並呼叫它。（它在內部呼叫 normalizeClass 和 normalizeStyle）

現在我們已經實作到這裡，讓我們看看它是如何工作的！
