...
我們已經有了基本元件系統插槽實作的執行時實作。\
但是，我們仍然無法在模板中處理插槽。

我們希望處理如下的 SFC：\
（雖然我們說 SFC，但實際上是模板編譯器的實作。）
...
但是，正如您可能已經從執行時實作中了解到的，這些都只是回呼函式。讓我們回顧一下以防萬一。
...
在模板中，`name="default"` 屬性可以省略，但在執行時，它仍然會被視為名為 `default` 的插槽。我們將在完成具名插槽的實作後實作預設插槽的編譯器。

## 實作編譯器（插槽定義）

像往常一樣，我們將實作解析和程式碼產生過程，但這次我們將處理插槽定義和插槽插入。

首先，讓我們專注於插槽定義。這是在子元件端表示為 `<slot name="my-slot"/>` 的部分。

在執行時，我們將準備一個名為 `renderSlot` 的輔助函式，它將透過元件實例（透過 `ctx.$slot`）插入的插槽及其名稱作為參數。原始碼將被編譯為如下內容：
...
現在我們已經到了這一點，下一步是實作轉換器來產生 `codegenNode`。\
...
我們將實作一個名為 `transformSlotOutlet` 的新轉換器。\
...
需要注意的一點是 `<slot />` 元素也會被 `transformElement` 捕獲，所以我們將新增一個實作，在遇到 `ElementTypes.SLOT` 時跳過它。
...
我們還沒有實作執行時函式 `renderSlot`，所以我們將最後做這件事來完成插槽定義的實作。

讓我們實作 `packages/runtime-core/helpers/renderSlot.ts`。
