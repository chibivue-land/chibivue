...
在 Vue 3.4 中，模板編譯器的內部實作進行了重大重構。到目前為止，我們在 chibivue 中實作的解析器是基於 Vue 3.3 及更早版本的架構。
...
這個狀態機 tokenizer 實作了**一致的 2 倍**解析速度提升！\
令人驚訝的是，僅僅透過避免正規表示式和前瞻搜尋，一個字元一個字元地順序處理，就能實作如此顯著的效能提升。

</KawaikoNote>

## 狀態機 Tokenizer

狀態機 tokenizer 根據目前狀態決定如何處理下一個字元。
...
本章介紹的狀態機 tokenizer 在 chibivue 中還沒有實作，但如果你有興趣，可以嘗試自己實作！\
